...
#include <iostream>
#include <fstream>
VertexBufferObject::VertexBufferObject(GLuint &index)
{
    id = index;
    rows = 0;
    cols = 0;
...
{//&id specifies the array in which the generated VAO names are stored, assign to id
    // id can be an array of GLint
...
//do i replace the parameters with MatrixXf array? Is MatrixXf automatically casted to double?
void VertexBufferObject::update(const Eigen::MatrixXf& M)
{
    assert(id != 0);
    //can it be "this.bind"
    glBindBuffer(GL_ARRAY_BUFFER, id);
    // M.size is the 1-d array size of V, M.data is the 1-d array of coordinates in V
        ...
        const std::string &name, VertexBufferObject& VBO) const
{
    ...
    //???
    glVertexAttribPointer(id, VBO.rows, GL_FLOAT, GL_FALSE, 0, 0);//(vPosition, 4, GL_FLOAT,GL_FALSE,0, BUFFER_OFFSET(0))c
    check_gl_error();
